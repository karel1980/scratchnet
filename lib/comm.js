...
function createServiceHandler(comm, conf) {
    ...
    var srv;
    if (conf.type == "peer") {
        srv = new PeerServiceHandler(conf);
...
function pollQueues(qs) {
    ...
    for (var name in qs) {
        var q = qs[name];
        poll += pollArgs( q._crnt, name + '-crnt-');
        poll += pollArgs( q._last, name + '-last-');
        poll += name + '-ptr '  + q._ptr  + '\n';
        poll += name + '-top '  + q._top  + '\n';
        poll += name + '-hasNext '  + q.hasNext() + '\n';
...
Communication.prototype.init = function() {
    ...
    //
    // called by scratch for blocks
    //
    var name="send";
    this.app.get('/' + name + '/:msg', function(req, res){
        var args = [req.param('msg')];
        sendMessage({message: name, args: args});
        res.setHeader('Content-Type', 'text/plain');
        res.send('ok');
    });

    this.app.get('/' + name + '-ack', function(req, res){
    ...
    //TODO rewrite as app.use with next() that checks the method
    ...
    // transfered by other through POST
    ...
    this.app.post('/receive', function(req, res){
        ...
        console.log("received message %s", content.msg);
        var q = self.getQueue(name);
        q.enqueue(content);
        console.log("state of queue[%s] = %j", name, q);
        self._error = null;
