...
function createServiceHandler(comm, conf) {
    var srv;
    console.log("TTT", JSON.stringify(conf))
    if (conf.type == "peer") {
...
Communication.prototype.init = function() {
    ...
    app.get('/crossdomain.xml', function(req, res){
        ...
        var xdomfile = path.join(__dirname,'static','crossdomain.xml');
...
PeerServiceHandler.prototype.getName = function() {
    return this.conf.name;
}

PeerServiceHandler.prototype.getId = function() {
    return this.conf.id;
}
...
PeerServiceHandler.prototype.addScratchBlockSpecsForMessage = function(blocks, name, msgConf, remoteid) {
    ...
    var blockName = util.format(msgName, remoteid).trim();
    ...
    for (var i=0; i<msgArgs.length; i++) {
        ...
        if (argCrntName) {
            blocks.push(["r", util.format(argCrntName,remoteid).trim(), name + "-crnt-" + arg.id]);
        }
        //add args-top
        var argLastName = arg.queue.last || arg.id;
        blocks.push(["r", util.format(argLastName,remoteid).trim(), name + "-last-" + arg.id]);
    }
    ...
    if (msgConf.queue) {
        ...
        if (!!ackName) {
            blocks.push([" ", util.format(ackName, remoteid).trim(), name+ "-ack"]);
        }
        //add ptr reporterblock
        var ptrName = msgConf.queue.ptr;
        if (!!ptrName) {
            blocks.push(["r", util.format(ptrName, remoteid).trim(), name+ "-ptr"]);
        }
        //add top reporterblock
        var topName = msgConf.queue.top;
        if (!!topName) {
            blocks.push(["r", util.format(topName, remoteid).trim(), name+ "-top"]);
        }
        //add boolean indicator reporterblock
        var hasnextName = msgConf.queue.hasnext;
        if (!!hasnextName) {
            blocks.push(["r", util.format(hasnextName, remoteid).trim(), name+ "-hasNext"]);
        }
    }
...
PeerServiceHandler.prototype.makeScratchExtensionSpec = function(localport, remoteid) {
    ...
    spec.filename = util.format("ext-%s-%s.s2e", this.id, remoteid);
    //TODO make blockspecs from conf - for now fake/hardcoded
    spec.extension = {
        "extensionName": util.format(this.conf.name, remoteid).trim(),
