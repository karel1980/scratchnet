...
var Manager = function(options) {
    ...
    this.local_catalog = options.local_catalog || {
      "chat": conf.get("service","chat-service"),
      "guess": conf.get("service","number-guess-game"),
    }
...
Manager.prototype.setupApp = function() {
    ...
    this.app.post("/invite", function(req, res) {
      console.log("Received an invitation with key", req.params('key'))
      self.invitations_in[key] = req.body
    ...
    this.app.post("/accepted/:key", function(req, res) {
      var key = req.params('key')
      console.log("Other end accepted our invitation with key", key)
      var invite = self.invitations_out[key]
      invite.accepted = true;
      invite.comm = self.createComm(self.invitations_out[key])
      res.json({bind:invite.comm})
    })
    ...
    this.app.post("/do_invite", function(req,res){
      ...
      console.log("Creating and sending invitation to ", other, "for", svcId)
      var key = self.id + "--" + other + "--" + svcId
      var invitation = { from: self.id, to: other, serviceId: svcId }
