...
var Manager = function(options) {
    ...
    // FIXME: conf should have a method to list all confs for a specific type
    this.local_catalog = options.local_catalog || {
      "chat-1.0": conf.get("service","chat-service"),
      "guess-1.0": conf.get("service","number-guess-game"),
    }
...
// with a function the output is    [ {key: key, value: fn(key,value), ... ]
...
Manager.prototype.setupApp = function() {
    ...
    this.app.post("/invite", function(req, res) {
      ...
      self.notification_queue.push({ type: "invitation", key: key})
      res.json({"ok":"ok"})
    })
    ...
    this.app.post("/accepted", function(req, res) {
      ...
      var bind = req.param('bind')
      var invite = self.invitations_out[key]

      console.log("Alice sees bob's acceptance of 'key'")

      // Since we're the initiator of this invitation we'll need
      // to start our comm and notify the other of the bind
      invite.comm = self.createComm(invite.key, invite.service, function(bind) {
        console.log("alice created a comm, adds invite to links")
        self.links[key] = invite
        request.post({
          uri: self.uriFor(invite.to, "/notify-bind"),
          json: { key:invite.key, bind: bind }
        })
      })
      res.json({"ok":"I am staring a comm, I'll notify you of the bind when it's done"})
      //OK to do this in the callback above?
      //TODO TEST
    })

    // called by other manager
    // A call here means we receive the bind which is the peer for our comm
    this.app.post("/notify-bind", function(req,res) {
      var key = req.param('key')
      var bind = req.param('bind')
      console.log("bob is notified of alice's comm, can now set peer for link ", key)
      self.links[key].comm.setPeer(bind)
    })
...
Manager.prototype.advertise = function(services) {
  console.log("Advertising catalog: ", JSON.stringify(services))
...
Manager.prototype.accept = function(key, callback) {
   console.log("bob accepting invitation with key", key)
   ...
   // initial replies
   invite.comm = this.createComm(invite.key, invite.service, function(bind) {
     console.log("bob created his comm, add invite to links with key ", key)
     self.links[key] = invite;
     request.post({
       uri: uri,
       json: { key: key, bind: bind }
     }, function(err,incoming,res) {
       console.log("bob done tellin alice he accepted")
       if (callback) {
         callback(invite)
       }
     }) 
